...
- Update the latest rates based on user's volumes
...
- startup_batch_update_fee: True: The user rate is automatically updated when the service is started
...
3. What are the functions of Orderly EVM Broker Fee Admin?
   ...
   - Adjust users' rates in batches based on the defined 30-day transaction volume rate level configuration
...
It is recommended to run this task no less than 00:10 UTC every day, and the update scope does not include fixed rate users (special_rate_whitelists).
